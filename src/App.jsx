...
function LoginPage({ onLogin }) {
  ...
  const handleLogin = () => {
    ...
    setError("");
    setTimeout(() => {
      onLogin({ name: username, role: "管理员" });
    }, 800);
  };
      ...
      <div className="fade-in" style={{
        ...
        <div style={{ textAlign: "center", marginBottom: 36 }}>
          <div style={{
            ...
            fontSize: 24, color: "#fff", fontWeight: 800, marginBottom: 16,
        ...
        <div style={{ marginBottom: 18 }}>
          ...
          <input
            ...
            style={{
              ...
              padding: "0 14px", fontSize: 14, color: "#e2e8f0",
            }}
          />
        </div>
...
/* ═══════════════════════════════════════════
   SIDEBAR
   ═══════════════════════════════════════════ */
function Sidebar({ activeMenu, setActiveMenu, expandedKey, setExpandedKey, collapsed, setCollapsed }) {
  return (
    <div style={{
            ...
            <div key={item.key}>
              <div
                onClick={() => {
                  ...
                  if (item.children) setExpandedKey(isExpanded ? null : item.key);
                }}
                style={{
                  display: "flex", alignItems: "center",
                  gap: 10,
                  padding: collapsed ? "11px 0" : "11px 22px",
                  justifyContent: collapsed ? "center" : "flex-start",
                  cursor: "pointer",
              ...
              >
                ...
                {!collapsed && item.children && (
                  <span style={{ fontSize: 10, opacity: 0.4, transform: isExpanded ? "rotate(90deg)" : "none", transition: "transform 0.2s" }}>▶</span>
                )}
              </div>
...
function TopBar({ user, onLogout, activeMenu }) {
  const current = MENU.find(m => m.key === activeMenu);
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 14 }}>
        ...
        {current && activeMenu !== "dashboard" && <>
          <span style={{ color: "#cbd5e1" }}>/</span>
          <span style={{ color: "#2563eb", fontWeight: 500 }}>{current.label}</span>
        </>}
      </div>
      <div style={{ display: "flex", alignItems: "center", gap: 18 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
          ...
          <span style={{ fontSize: 13, color: "#4b5563" }}>{user.name}</span>
        </div>
...
function FilterBar() {
  const fields = [
    ...
    { label: "季节", placeholder: "全部", w: 90, select: true },
    { label: "波段", placeholder: "全部", w: 90, select: true },
    { label: "款式类别", placeholder: "全部", w: 110, select: true },
    { label: "款号", placeholder: "", w: 110 },
    { label: "客户PPO", placeholder: "", w: 110 },
    { label: "客户", placeholder: "全部", w: 100, select: true },
    { label: "加工厂", placeholder: "全部", w: 100, select: true },
    { label: "订单类型", placeholder: "全部", w: 100, select: true },
    { label: "加工类型", placeholder: "全部", w: 100, select: true },
    { label: "客户品牌", placeholder: "全部", w: 100, select: true },
    { label: "设计师", placeholder: "全部", w: 100, select: true },
  ];
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 10, alignItems: "flex-end" }}>
        {fields.map((f, i) => (
          <div key={i} style={{ display: "flex", flexDirection: "column", gap: 3 }}>
            ...
            {f.select ? (
              <select style={{ ...fieldStyle, width: f.w }}><option>{f.placeholder}</option></select>
            ) : (
              <input placeholder={f.placeholder} style={{ ...fieldStyle, width: f.w }} />
            )}
          </div>
        ))}
...
function StatusBadge({ status, days }) {
  const map = {
    overdue: { bg: "#ef4444", text: `逾期${days}天` },
    inProgress: { bg: "#3b82f6", text: `进行${days}天` },
    notStarted: { bg: "#f59e0b", text: `剩余${days}天` },
    completed: { bg: "#22c55e", text: "已完成" },
  };
        ...
        <div style={{ flex: 1, fontSize: 12.5, color: "#4b5563", lineHeight: 1.9 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 4 }}>
            <div style={{ flex: 1, height: 5, background: "#e5e7eb", borderRadius: 3 }}>
              <div style={{
                width: `${order.progress}%`, height: "100%", borderRadius: 3,
                background: order.progress > 80 ? "#22c55e" : order.progress > 50 ? "#3b82f6" : "#f59e0b",
      ...
      <div style={{ display: "flex", borderTop: "1px solid #f1f5f9", background: "#fafbfc" }}>
        {TABS.map((tab, i) => (
          <div key={tab} style={{
            ...
            fontSize: 12, color: i === 0 ? "#2563eb" : "#b0b8c4",
    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div style={{ background: "#fff", borderRadius: 10, padding: 20, border: "1px solid #e2e8f0" }}>
          <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 14 }}>快捷操作</div>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
            {["新建订单", "新建款式", "生产排期", "质检报告", "库存查询", "财务报表"].map(a => (
              <button key={a} style={{
          ...
          {[
            ...
            <div key={i} style={{
              ...
              <span style={{
                ...
                background: item.t === "success" ? "#22c55e" : item.t === "warning" ? "#ef4444" : "#3b82f6",
              }} />
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 16 }}>
        {/* Left summary */}
        <div style={{
          width: 186, background: "#fff", borderRadius: 10,
          border: "1px solid #e2e8f0", padding: "14px 0",
          flexShrink: 0, alignSelf: "flex-start",
        }}>
          <div style={{ fontSize: 14, fontWeight: 600, padding: "0 16px 12px", borderBottom: "1px solid #f1f5f9" }}>
            生产汇总列表
          </div>
          {summaryItems.map((it, i) => (
            <div key={i} style={{
              padding: "10px 16px", fontSize: 13,
              color: it.active ? "#2563eb" : "#4b5563",
              fontWeight: it.active ? 600 : 400,
              background: it.active ? "#eff6ff" : "transparent",
              cursor: "pointer",
              borderLeft: it.active ? "3px solid #2563eb" : "3px solid transparent",
              transition: "all 0.15s",
            }}
            ...
            >
              <div>{it.label}</div>
              <div style={{ fontSize: 11, color: "#94a3b8", marginTop: 2 }}>{it.sub}</div>
            </div>
          ))}
        </div>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{
            ...
            gap: 8, padding: "14px 0", fontSize: 13, color: "#6b7280",
          }}>
            <span>共 11 条</span>
            <select style={{ height: 28, borderRadius: 4, border: "1px solid #d1d5db", fontSize: 12, padding: "0 6px" }}>
              <option>20条/页</option>
            </select>
            {["‹", "1", "›"].map((t, i) => (
              <span key={i} style={{
                ...
                display: "inline-flex", alignItems: "center", justifyContent: "center",
                background: t === "1" ? "#2563eb" : "#fff",
                color: t === "1" ? "#fff" : "#6b7280",
                border: t === "1" ? "none" : "1px solid #d1d5db",
...
export default function App() {
  ...
  const currentLabel = MENU.find(m => m.key === activeMenu)?.label || "";
  const pages = {
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0 }}>
        <TopBar user={user} onLogout={() => setUser(null)} activeMenu={activeMenu} />
